...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `F001` | 简历数据录入 | 用户在配置页录入并本地持久化全部简历数据 | 一切填写能力的前提；本身即结构化简历存档 | `P0` | 无 | `REVIEW` | `CONFIRMED` | 配置页分区表单（基本信息、教育[]、工作[]、项目[]、技能、自我评价、自定义项）+ 存储层封装；Gate：SPEC/UI/TEST DESIGN 全部 PASS（2026-08-27）；实现完成，22 项自动化测试通过 |
